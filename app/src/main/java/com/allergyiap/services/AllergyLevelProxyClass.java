...
public class AllergyLevelProxyClass {
    ...
    public static List<AllergyLevelEntity> getLevels(int stationId) throws Exception {
        ...
        Context context = DBHelper.getCurrentContext();
        DBHelper db = DBHelper.getDBHelper(context);
        JSONArray t = db.getQuery("SELECT *, AS allergy_name FROM allergy_level INNER JOIN allergy ON allergy.idallergy = allergy_level.allergy_idallergy INNER JOIN stations ON stations.name = allergy_level.station WHERE station.id=" + stationString + " AND date_start <= DATE('NOW') AND DATE('NOW') <= date_end ");
        if (t.length() == 0) {
            InputStream s = null;
            ...
            t = db.getQuery("SELECT *, AS allergy_name FROM allergy_level INNER JOIN allergy ON allergy.idallergy = allergy_level.allergy_idallergy INNER JOIN stations ON stations.name = allergy_level.station WHERE station.id=" + stationString + " AND date_start <= DATE('NOW') AND DATE('NOW') <= date_end ");
        }
